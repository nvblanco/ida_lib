...
class GaussianNoisePipeline(PipelineOperation):
    ...
    def apply_to_image_if_probability(self, img: np.ndarray) -> np.ndarray:
        if PipelineOperation.apply_according_to_probability(self):
            img = color_functional._apply_gaussian_noise(img, self.var)
        return img
...
class SaltAndPepperNoisePipeline(PipelineOperation):
    ...
    def apply_to_image_if_probability(self, img: np.ndarray) -> np.ndarray:
        if PipelineOperation.apply_according_to_probability(self):
            img = color_functional._apply_salt_and_pepper_noise(img, self.amount, self.s_vs_p)
        return img
...
class SpekleNoisePipeline(PipelineOperation):
    ...
    def apply_to_image_if_probability(self, img: np.ndarray) -> np.ndarray:
        if PipelineOperation.apply_according_to_probability(self):
            img = color_functional._apply_spekle_noise(img)
        return img
...
class PoissonNoisePipeline(PipelineOperation):
    ...
    def apply_to_image_if_probability(self, img: np.ndarray) -> np.ndarray:
        if PipelineOperation.apply_according_to_probability(self):
            img = color_functional._apply_poisson_noise(img)
        return img
...
class GaussianBlurPipeline(PipelineOperation):
    ...
    def apply_to_image_if_probability(self, img: np.ndarray) -> np.ndarray:
        if PipelineOperation.apply_according_to_probability(self):
            img = color_functional.apply_gaussian_blur(img, blur_size=self.blur_size)
        return img
...
class BlurPipeline(PipelineOperation):
    ...
    def  apply_to_image_if_probability(self, img: np.ndarray) -> np.ndarray:
        if PipelineOperation.apply_according_to_probability(self):
            img = color_functional._apply_blur(img, blur_size=self.blur_size)
        return img
