...
def numpy_float_all_elements_item():
    ...
    return {'image': img, 'keypoints': points(), 'segmap': segmap(img, dtype=np.float),
            'mask': mask(img, dtype=np.float),
...
def numpy_float_all_elements_batch():
    ...
    data = {'image': img, 'keypoints': points(), 'segmap': segmap(img, dtype=np.float),
            'mask': mask(img, dtype=np.float),
            'heatmap': heatmap(img, dtype=np.float)}
    batch = [data.copy() for _ in range(samples)]
    return batch
