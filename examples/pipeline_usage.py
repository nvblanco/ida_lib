...
#Generate the input item of the pipeline. Its very importantto name each element with its data type so that the pipeline understands them
#if the item contains more than one element of each type, just number them like mask1 and mask2
data = { 'image': img, 'keypoints': random_coordinates,'mask1': mask_example1, 'mask2': mask_example2, 'heatmap': heatmap_complete}

#For this example we are going to use the same identical input element but repeated n times to create a batch so we can see the different transformations
samples = 100
